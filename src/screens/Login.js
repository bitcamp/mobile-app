...
export default class Login extends Component<Props> {
  ...
  async sendReceivedCode(code) {
    ...
    try {
      const email = this.state.savedEmail;
      const response = await mockFetch(url, {
        method: "POST",
        headers: {
          Accept: "application/json",
      ...
      if (response.status == 200) {
        ...
        userFavoritedEvents = this.processUserEvents(
  ...
  render() {
    return (
      <KeyboardShift>
        <PadContainer
          style={
            this.state.keyboardShown ? styles.subSection2 : styles.subSection
          }
        >
          <Heading style={{ paddingBottom: 20 }}>{this.state.greeting}</Heading>
          <SubHeading>{this.state.instruction}</SubHeading>
          <TextInput
